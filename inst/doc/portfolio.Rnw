...
Communications and 10\% short Japan. In future versions of the
package, we plan on adding other analytics as well as plotting
methods.
...
for each time period separately, saving those results, and the
